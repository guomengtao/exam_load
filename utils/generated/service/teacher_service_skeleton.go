...
type TeacherServiceSkeleton struct {
	db *gorm.DB
}

func NewTeacherServiceSkeleton(db *gorm.DB) *TeacherServiceSkeleton {
	return &TeacherServiceSkeleton{
		db: db,
	}
}

// GetCount 返回数据库中该模型的总记录数
func (s *TeacherServiceSkeleton) GetCount() (int64, []ErrorResponse) {
	var count int64
	if err := s.db.Model(&models.Teacher{}).Count(&count).Error; err != nil {
...
func (s *TeacherServiceSkeleton) List(page int, pageSize int) ([]*models.Teacher, int64, []ErrorResponse) {
	var items []*models.Teacher
	var total int64

	offset := (page - 1) * pageSize

	err := s.db.Model(&models.Teacher{}).
		Count(&total).
		Limit(pageSize).
		Offset(offset).
		Find(&items).Error
...
// BatchCreate 批量创建记录
func (s *TeacherServiceSkeleton) BatchCreate(items []*models.Teacher) (int, []ErrorResponse) {
	if len(items) == 0 {
...
// BatchUpdate 批量更新记录
func (s *TeacherServiceSkeleton) BatchUpdate(items []*models.Teacher) (int, []ErrorResponse) {
	if len(items) == 0 {
		return 0, []ErrorResponse{NewErrorResponse(400, "empty items", "")}
	}
...
// BatchDelete 批量删除记录
func (s *TeacherServiceSkeleton) BatchDelete(ids []int64) (int, []ErrorResponse) {
	if len(ids) == 0 {
	...
	// 删除主表数据
	if err := tx.Delete(&models.Teacher{}, ids).Error; err != nil {
		if err := tx.Rollback().Error; err != nil {
