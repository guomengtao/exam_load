...
// FileInfoSkeleton 控制器骨架
type FileInfoSkeleton struct {
	biz *biz.FileInfoBiz
}

// NewFileInfoSkeleton 创建新的FileInfoSkeleton实例
func NewFileInfoSkeleton(biz *biz.FileInfoBiz) *FileInfoSkeleton {
	return &FileInfoSkeleton{biz: biz}
}

// CountHandler 获取记录总数
func (s *FileInfoSkeleton) CountHandler(c *gin.Context) {
...
// ListHandler 获取记录列表
func (s *FileInfoSkeleton) ListHandler(c *gin.Context) {
...
func (s *FileInfoSkeleton) BatchCreateHandler(c *gin.Context) {
...
func (s *FileInfoSkeleton) BatchUpdateHandler(c *gin.Context) {
	var items []*models.FileInfo
	if err := c.ShouldBindJSON(&items); err != nil {
		utils.Error(c, "请求参数错误: "+err.Error())
		return
...
func (s *FileInfoSkeleton) BatchDeleteHandler(c *gin.Context) {
	var ids []int
